...
def run_policy_server(args: argparse.Namespace) -> None:
    ...
    if hasattr(policy_cfg, "temporal_ensemble_coeff"):
        # Force ACT serving to use action-chunk queue behavior instead of temporal ensembling.
        policy_cfg.temporal_ensemble_coeff = None
    if hasattr(policy_cfg, "chunk_size") and hasattr(policy_cfg, "n_action_steps"):
        logging.info(
            "Serving with action-chunk mode: chunk_size=%s, n_action_steps=%s, temporal_ensemble_coeff=%s",
            getattr(policy_cfg, "chunk_size", None),
            getattr(policy_cfg, "n_action_steps", None),
            getattr(policy_cfg, "temporal_ensemble_coeff", None),
        )
...
def _apply_cli_to_module_constants(args: argparse.Namespace) -> None:
    ...
    global SERVE_HOST, SERVE_PORT, SERVE_POLICY_CHECKPOINT
